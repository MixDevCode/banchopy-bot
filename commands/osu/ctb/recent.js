...
module.exports = {
    name: 'catchrecent',
    aliases: ['catchrs', 'ctbr', 'ctbrs'],
    description: 'Get the most recent catch play of a user',
    usage: 'catchrecent [username...]',
    options: [
    ...
    run: async(client, message, args) => {
        if (!args[0]) {
            ...
            const embed = new MessageEmbed({
                color: '#0099ff',
                author: {
                    name: `Most recent osu!Catch play for ${playerInfo.player.info.name}`,
            ...
            if (parameters.contains == true && parameters.hasUser == true) {
                ...
                const embed = new MessageEmbed({
                    color: '#0099ff',
                    author: {
                        name: `Top 5 most recent osu!Catch plays for ${playerInfo.player.info.name} sorted by pp`,
                        icon_url: `${botConfig.server.avatarurl}${playerInfo.player.info.id}?${Math.floor(Math.random()*9999)}`
                    },
