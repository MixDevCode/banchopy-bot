...
module.exports = {
    name: 'catchtop',
    aliases: ['ctbtop'],
    description: "Get user's top osu catch plays.",
    usage: 'catchtop [username...]',
    options: [
        ...
        if (!args[0]) {
            ...
            if(data.length > 1) {
                const embeds = data.map((x) => {
                    return new MessageEmbed()
                        .setAuthor({ name: `Top ${bestInfo.scores.length} osu!Catch plays for ${playerInfo.player.info.name}`, iconURL: `${botConfig.server.avatarurl}${playerInfo.player.info.id}?${Math.floor(Math.random()*9999)}` })
                        .setColor("#0099ff")
                        .setDescription(x)
                ...
                const embed = new MessageEmbed({
                    color: '#0099ff',
                    author: {
                        name: `Top ${bestInfo.scores.length} osu!Catch plays for ${playerInfo.player.info.name}`,
            ...
            if (parameters.contains == true && parameters.hasUser == true) {
                ...
                const embed = new MessageEmbed({
                    color: '#0099ff',
                    author: {
                        name: `Top 5 osu!Catch plays for ${playerInfo.player.info.name} sorted by play time`,
                        icon_url: `${botConfig.server.avatarurl}${playerInfo.player.info.id}?${Math.floor(Math.random()*9999)}`
                    },
                    description: description,
                    thumbnail: {
                ...
                const embed = new MessageEmbed({
                    color: '#0099ff',
                    author: {
                        name: `Random best osu!Catch play for ${playerInfo.player.info.name}`,
                        icon_url: `${botConfig.server.avatarurl}${playerInfo.player.info.id}?${Math.floor(Math.random()*9999)}`
                    },
                ...
                const embed = new MessageEmbed({
                    color: '#0099ff',
                    author: {
                        name: `Top osu!Catch play n° ${index+1} for ${playerInfo.player.info.name}`,
                        icon_url: `${botConfig.server.avatarurl}${playerInfo.player.info.id}?${Math.floor(Math.random()*9999)}`
                    },
                ...
                const embed = new MessageEmbed({
                    color: '#0099ff',
                    author: {
                        name: `Top 5 osu!Catch plays for ${playerInfo.player.info.name}`,
                        icon_url: `${botConfig.server.avatarurl}${playerInfo.player.info.id}?${Math.floor(Math.random()*9999)}`
                    },
                    description: description,
                    thumbnail: {
