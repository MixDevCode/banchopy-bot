    ...
    run: async(client, message, args) => {
        if (!args[0]) {
            ...
            let {star_avg, aim_avg, speed_avg, acc_avg,
                finger_control_avg, calc_count} = await calcPlayer(message, userSchema.username, 3)
            ...
            const path_suffix = 'akatsuki_mania'
            ...
            function card_stat(onlycard) {
                ...
                let skillname = '', skillnumber = '', stat_number_x = 150;
                if (onlycard) {
                    for (let num of modenum_skill[3].skill) {
                        if (num == 0) skillname += `${skill_name_holder[3]}: ${skill_holder[num]}\n`;
                        else skillname += `${skill_name_holder[num]}: ${skill_holder[num]}\n`;
                    }
                    return {skillname: skillname, stat_number_x: stat_number_x}
                } else {
                    for (let num of modenum_skill[3].skill) {
                        if (num == 0) skillname += `${skill_name_holder[3]}:\n`;
            ...
            if(playerInfo == undefined || playerInfo.length == 0) return;
            let {star_avg, aim_avg, speed_avg, acc_avg,
                finger_control_avg, calc_count} = await calcPlayer(message, username, 3)
