...
module.exports = {
    name: 'maniarecent',
    aliases: ['mrs', 'mr'],
    description: 'Get the most recent mania play of a user',
    usage: 'maniarecent [username...]',
    options: [
        ...
        if (!args[0]) {
            ...
            if (parameters.contains == true && parameters.hasUser == true) {
                ...
                const embed = new MessageEmbed({
                    color: '#0099ff',
                    author: {
                        name: `Top 5 most recent osu!Mania plays for ${playerInfo.player.info.name} sorted by pp`,
                        icon_url: `${botConfig.server.avatarurl}${playerInfo.player.info.id}?${Math.floor(Math.random()*9999)}`
                    },
