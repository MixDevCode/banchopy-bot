...
module.exports = {
    name: 'relax',
    aliases: ['rx'],
    description: 'Get an osu!relax profile.',
    usage: 'relax [username...]',
    options: [
    ...
    run: async(client, message, args) => {
        if(!args[0]){
            ...
            const embed = new MessageEmbed({
                color: '#0099ff',
                description: `**osu!Relax status for: [${playerInfo.player.info.name}](${botConfig.server.domainurl}u/${playerInfo.player.info.id})**`,
                fields: [{
                    name: 'Performance',
                    value: `--- **${playerInfo.player.stats[4].pp}pp**\n**Global Rank** #${playerInfo.player.stats[4].rank} (:flag_${playerInfo.player.info.country}: #${playerInfo.player.stats[4].country_rank})\n**Accuracy:** ${Number((playerInfo.player.stats[4].acc).toFixed(2))}%\n**Playcount:** ${playerInfo.player.stats[4].plays}\n**Level:** ${level(playerInfo.player.stats[4].rscore)}`,
                    inline: true
                }, {
                    name: 'Rank',
                    value: `${ranks("SSH")}: ${playerInfo.player.stats[4].xh_count}\n${ranks("SS")}: ${playerInfo.player.stats[4].x_count}\n${ranks("SH")}: ${playerInfo.player.stats[4].sh_count}\n${ranks("S")}: ${playerInfo.player.stats[4].s_count}\n${ranks("A")}: ${playerInfo.player.stats[4].a_count}`, 
                    inline: true
                }],
                footer: {
                    text: status,
