...
module.exports = {
    name: 'relaxrecent',
    aliases: ['rxr', 'relaxr'],
    description: 'Get the most recent relax play of a user',
    usage: 'relaxrecent [username...]',
    options: [
    ...
    run: async(client, message, args) => {
        if (!args[0]) {
            ...
            const embed = new MessageEmbed({
                color: '#0099ff',
                author: {
                    name: `Most recent osu!Relax play for ${playerInfo.player.info.name}`,
            ...
            if (parameters.contains == true && parameters.hasUser == true) {
                ...
                const embed = new MessageEmbed({
                    color: '#0099ff',
                    author: {
                        name: `Top 5 most recent osu!Relax plays for ${playerInfo.player.info.name} sorted by pp`,
                        icon_url: `${botConfig.server.avatarurl}${playerInfo.player.info.id}?${Math.floor(Math.random()*9999)}`
                    },
