...
module.exports = {
    name: 'relaxtop',
    aliases: ['rxtop'],
    description: "Get user's top osu relax plays.",
    usage: 'relaxtop [username...]',
    options: [
        ...
        if (!args[0]) {
            ...
            if(data.length > 1) {
                const embeds = data.map((x) => {
                    return new MessageEmbed()
                        .setAuthor({ name: `Top ${bestInfo.scores.length} osu!Relax plays for ${playerInfo.player.info.name}`, iconURL: `${botConfig.server.avatarurl}${playerInfo.player.info.id}?${Math.floor(Math.random()*9999)}` })
                        .setColor("#0099ff")
                        .setDescription(x)
                ...
                const embed = new MessageEmbed({
                    color: '#0099ff',
                    author: {
                        name: `Top ${bestInfo.scores.length} osu!Relax plays for ${playerInfo.player.info.name}`,
            ...
            if (parameters.contains == true && parameters.hasUser == true) {
                ...
                const embed = new MessageEmbed({
                    color: '#0099ff',
                    author: {
                        name: `Top 5 osu!Relax plays for ${playerInfo.player.info.name} sorted by play time`,
                        icon_url: `${botConfig.server.avatarurl}${playerInfo.player.info.id}?${Math.floor(Math.random()*9999)}`
                    },
                    description: description,
                    thumbnail: {
                ...
                const embed = new MessageEmbed({
                    color: '#0099ff',
                    author: {
                        name: `Random best osu!Relax play for ${playerInfo.player.info.name}`,
                        icon_url: `${botConfig.server.avatarurl}${playerInfo.player.info.id}?${Math.floor(Math.random()*9999)}`
                    },
                ...
                const embed = new MessageEmbed({
                    color: '#0099ff',
                    author: {
                        name: `Top osu!Relax play n° ${index+1} for ${playerInfo.player.info.name}`,
                        icon_url: `${botConfig.server.avatarurl}${playerInfo.player.info.id}?${Math.floor(Math.random()*9999)}`
                    },
                ...
                if(data.length > 1) {
                    const embeds = data.map((x) => {
                        return new MessageEmbed()
                            .setAuthor({ name: `${bestInfo.scores.length} osu!Relax plays for ${playerInfo.player.info.name} matching "${search}"`, iconURL: `${botConfig.server.avatarurl}${playerInfo.player.info.id}?${Math.floor(Math.random()*9999)}` })
                            .setColor("#0099ff")
                            .setDescription(x)
                            .setThumbnail(`${botConfig.server.avatarurl}${playerInfo.player.info.id}?${Math.floor(Math.random()*9999)}`);
                    })
                    ...
                    const embed = new MessageEmbed({
                        color: '#0099ff',
                        author: {
                            name: `${bestInfo.scores.length} osu!Relax plays for ${playerInfo.player.info.name} matching "${search}"`,
                            icon_url: `${botConfig.server.avatarurl}${playerInfo.player.info.id}?${Math.floor(Math.random()*9999)}`
                        },
                        description: description,
                        thumbnail: {
