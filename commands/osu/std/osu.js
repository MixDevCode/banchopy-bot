    ...
    run: async(client, message, args) => {
        if(!args[0]){
            ...
            const embed = new MessageEmbed({
                color: '#0099ff',
                description: `**osu!Standard status for: [${playerInfo.player.info.name}](${botConfig.server.domainurl}u/${playerInfo.player.info.id})**`,
                fields: [{
                    name: 'Performance',
                    value: `--- **${playerInfo.player.stats[0].pp}pp**\n**Global Rank** #${playerInfo.player.stats[0].rank} (:flag_${playerInfo.player.info.country}: #${playerInfo.player.stats[0].country_rank})\n**Accuracy:** ${Number((playerInfo.player.stats[0].acc).toFixed(2))}%\n**Playcount:** ${playerInfo.player.stats[0].plays}\n**Level:** ${level(playerInfo.player.stats[0].rscore)}`,
                    inline: true
                }, {
                    name: 'Rank',
                    value: `${ranks("SSH")}: ${playerInfo.player.stats[0].xh_count}\n${ranks("SS")}: ${playerInfo.player.stats[0].x_count}\n${ranks("SH")}: ${playerInfo.player.stats[0].sh_count}\n${ranks("S")}: ${playerInfo.player.stats[0].s_count}\n${ranks("A")}: ${playerInfo.player.stats[0].a_count}`, 
                    inline: true
                }],
                footer: {
                    text: status,
