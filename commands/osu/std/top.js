        ...
        if (!args[0]) {
            ...
            if (parameters.contains == true && parameters.hasUser == true) {
                ...
                if(data.length > 1) {
                    const embeds = data.map((x) => {
                        return new MessageEmbed()
                            .setAuthor({ name: `${bestInfo.scores.length} osu!Standard plays for ${playerInfo.player.info.name} matching "${search}"`, iconURL: `${botConfig.server.avatarurl}${playerInfo.player.info.id}?${Math.floor(Math.random()*9999)}` })
                            .setColor("#0099ff")
                            .setDescription(x)
                            .setThumbnail(`${botConfig.server.avatarurl}${playerInfo.player.info.id}?${Math.floor(Math.random()*9999)}`);
                    })
                    ...
                    const embed = new MessageEmbed({
                        color: '#0099ff',
                        author: {
                            name: `${bestInfo.scores.length} osu!Standard plays for ${playerInfo.player.info.name} matching "${search}"`,
                            icon_url: `${botConfig.server.avatarurl}${playerInfo.player.info.id}?${Math.floor(Math.random()*9999)}`
                        },
                        description: description,
                        thumbnail: {
