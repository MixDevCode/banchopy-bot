...
module.exports = {
    name: 'taikorecent',
    aliases: ['taikors', 'taikor', 'trs', 'tr'],
    description: 'Get the most recent taiko play of a user',
    ...
    run: async(client, message, args) => {
        if (!args[0]) {
            ...
            const embed = new MessageEmbed({
                color: '#0099ff',
                author: {
                    name: `Most recent osu!Taiko play for ${playerInfo.player.info.name}`,
            ...
            if (parameters.contains == true && parameters.hasUser == true) {
                ...
                const embed = new MessageEmbed({
                    color: '#0099ff',
                    author: {
                        name: `Top 5 most recent osu!Taiko plays for ${playerInfo.player.info.name} sorted by pp`,
                        icon_url: `${botConfig.server.avatarurl}${playerInfo.player.info.id}?${Math.floor(Math.random()*9999)}`
                    },
